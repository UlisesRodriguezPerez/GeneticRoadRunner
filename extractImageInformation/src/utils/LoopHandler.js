class LoopHandler {
	...
	showDrawProcess() {
		...
		if ( roadRunnerGenerator.ready ) {
			push();
			roadRunnerGenerator.showProcess(); // where the magic happens
			pop();
		}
	...
	saveData() {
		...
		roadRunnerGenerator.newRoadRunner.info = JSON.parse( JSON.stringify( this.inputHandler.data ));
		//roadRunnerGenerator.imageAnalyzer.data = JSON.parse( JSON.stringify( this.inputHandler.data ));
	}
